...
app.post("/invite", async function (req, res) {
  ...
  const welcomeUrl = `${
    process.env.ENVIRONMENT === "DEV" ? "http" : "https"
  }://${invitationParams.host}.${process.env.APP_HOST}/welcome?group=${
    invitationParams.group
  }&ticket=${signInTicket.token}`;
  await axios.post(
    `${process.env.CLERK_BACKEND_API_URL}/emails`,
    {
      ...
      body: `Welcome to ${
        hostReadableNames[invitationParams.host]
      }! Sign in to your account here: ${welcomeUrl}`,
    },
    {
      headers: {
...
async function addExistingUser({ userId, host, orgId, group }) {
  ...
  const membership = memberships.find(
    ({ organization }) => organization.slug === host
  );
